...
public:
  ...
  void change_qty(QuantityDelta delta);
  ...
  Quantity reserve(QuantityDelta reserved);

private:
  OrderPtr order_;
  Quantity open_qty_;
  QuantityDelta reserved_;
...
template <class OrderPtr>
Quantity
OrderTracker<OrderPtr>::reserve(QuantityDelta reserved)
{
  reserved_ += reserved;
...
{
  if ((delta < 0 && 
      (QuantityDelta)open_qty_ < std::abs(delta))) {
    throw 
